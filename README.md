...
- 2026-03-08 UTC: eliminated all `filter: invert()` CSS rules following reports that dark mode rendered the dashboard as an inverted image; dark-mode icons now use `color` only, and a regression test has been added.
- 2026-03-08 UTC: dark-mode class is now applied server‑side based on a user preference flag, and the UI also automatically toggles when the visitor’s system prefers a dark color scheme.  This removes any dependence on client-side JS running correctly and makes Google‑style dark mode consistent across the site.
