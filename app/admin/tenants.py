...
def tenant_overview():
    ...
    memberships = TenantMembership.query.order_by(TenantMembership.created_at.desc()).all()
    return render_template("admin_tenants.html", tenants=tenants, memberships=memberships)
...
def tenant_members(tenant_id: int):
    ...
    if form.validate_on_submit():
        ...
        role = form.role.data
        active = bool(form.is_active.data)
        membership = TenantMembership.query.filter_by(tenant_id=tenant_id, user_id=user_id).first()
        if not membership:
    ...
    return render_template("admin_tenant_members.html", tenant=tenant, memberships=memberships, form=form)


@admin_bp.route("/tenants/<int:tenant_id>/members/<int:member_id>/delete", methods=["POST"])
@login_required
