...
def ensure_user_tenant_membership(user: User | None) -> Tenant | None:
    ...
    if not membership:
        ...
        changed = True
    elif getattr(user, "is_admin", False) and membership.role not in {
        "tenant_admin",
        "platform_admin",
    }:
...
def get_user_membership(
    user: User | None, tenant_id: int | None = None
    ...
    try:
        return TenantMembership.query.filter_by(
            user_id=user.id, tenant_id=target_id, is_active=True
        ).first()
    except Exception:
        return None
...
def user_has_permission(
    user: User | None, permission: str, tenant_id: int | None = None
) -> bool:
        ...
        try:
            if getattr(current_user, "is_authenticated", False):
                membership = get_user_membership(
                    current_user, getattr(tenant, "id", None)
                )
        except Exception:
            membership = None
        return {
    ...
    if not _TENANT_EVENTS_REGISTERED:
        ...
        def _assign_tenant_before_flush(session_obj, flush_context, instances):
            ...
            for obj in session_obj.new:
                if (
                    isinstance(obj, TenantScopedMixin)
