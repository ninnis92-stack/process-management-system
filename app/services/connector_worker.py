...
def process_pending_integration_events(limit: int = 20) -> int:
    ...
    try:
        ...
        events = (
            ...
            .order_by(IntegrationEvent.next_retry_at.asc().nullsfirst(), IntegrationEvent.created_at.asc())
            .limit(limit)
            .all()
        )
    except Exception:
    ...
    for ev in events:
        try:
            ...
            if kind == "webhook":
                ...
                try:
                    if provider_key:
                        candidates = IntegrationConfig.query.filter_by(kind="webhook", enabled=True).all()
                        for c in candidates:
                            try:
                            ...
                            if parsed.get("key") == provider_key or parsed.get("name") == provider_key:
                                cfg = c
                                cfg_parsed = parsed
                                break
                    ...
                    if cfg is None:
                        dept = None
                        try:
                            dept = (ev.metadata_json or {}).get("department") or (payload or {}).get("request", {}).get("department")
                        except Exception:
                            dept = None
                        if dept:
                            cfg = IntegrationConfig.query.filter_by(kind="webhook", department=dept, enabled=True).first()
                            if cfg:
                                try:
                    ...
                    if cfg is None:
                        cfg = IntegrationConfig.query.filter_by(kind="webhook", enabled=True).first()
                        if cfg:
                            try:
                                cfg_parsed = json.loads(cfg.config or "{}")
                            except Exception:
                ...
                webhook_url = (payload or {}).get("webhook_url") or (payload or {}).get("slack_webhook")
                if not webhook_url and cfg_parsed:
                    webhook_url = cfg_parsed.get("url") or cfg_parsed.get("webhook_url") or cfg_parsed.get("endpoint")
                # resolve secret references via secret_store (env/vault/aws)
                if isinstance(webhook_url, dict) or (isinstance(webhook_url, str) and (webhook_url.startswith("env:") or webhook_url.startswith("vault:") or webhook_url.startswith("aws_secrets_manager:"))):
                ...
                # slack events
                elif ename.startswith("automation.slack") or payload.get("channel") or payload.get("slack_webhook"):
                    ...
                    if not webhook and provider_key:
                        try:
                            cfgs = IntegrationConfig.query.filter_by(kind="webhook", enabled=True).all()
                            for c in cfgs:
                                try:
                                    parsed = json.loads(c.config or "{}")
                                except Exception:
                                    parsed = {}
                                if parsed.get("key") == provider_key or parsed.get("name") == provider_key:
                                    webhook = parsed.get("url") or parsed.get("webhook_url")
                                    break
                        except Exception:
                            webhook = None

                    # If webhook looks like a secret-ref, resolve it
                    if isinstance(webhook, dict) or (isinstance(webhook, str) and (webhook.startswith("env:") or webhook.startswith("vault:") or webhook.startswith("aws_secrets_manager:"))):
