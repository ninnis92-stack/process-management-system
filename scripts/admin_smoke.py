...
def main():
    try:
        ...
        paths = ['/admin/special_email', '/admin/feature_flags', '/admin/users', '/admin/assignments', '/metrics', '/metrics/json']
        for p in paths:
            try:
                rr = session.get(urljoin(BASE, p), timeout=10)
                print(f'GET {p} ->', rr.status_code, 'len=', len(rr.text) if rr.text else 0)
