...
def run_flow():
    ...
    def login(client, email):
        resp = client.post('/auth/login', data={'email': email, 'password': 'password123'}, follow_redirects=True)
    ...
    resp = client_b.post(f'/requests/{req_id}/transition', data={'to_status': 'B_IN_PROGRESS', 'submission_summary': 'Picking up work'}, follow_redirects=True)
    print('B -> B_IN_PROGRESS ->', resp.status_code)

    # B mark requires C review and send to pending
    resp = client_b.post(f'/requests/{req_id}/transition', data={'to_status': 'PENDING_C_REVIEW', 'requires_c_review': 'y', 'submission_summary': 'Ready for C review'}, follow_redirects=True)
    ...
    resp = client_c.post(f'/requests/{req_id}/transition', data={'to_status': 'C_APPROVED', 'submission_summary': 'Approved by C'}, follow_redirects=True)
