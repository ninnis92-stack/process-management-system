...
def _check_api_key(req):
	key = req.headers.get('X-Api-Key') or (req.headers.get('Authorization') or '').replace('Bearer ', '').strip()
	if not key:
		return None
...
def api_templates():
	...
	for t in templates:
		...
		for f in t.fields.order_by(FormField.order.asc()).all():
			opts = [dict(value=o.value, label=o.label) for o in f.options.order_by(FormFieldOption.order.asc()).all()]
			tfields.append(dict(name=f.name, label=f.label, type=f.field_type, required=bool(f.required), hint=f.hint, options=opts))
		out.append(dict(id=t.id, name=t.name, description=t.description, fields=tfields))
