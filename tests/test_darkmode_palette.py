...
def test_dark_mode_palette_values():
    ...
    assert (
        "--page-bg: radial-gradient(circle at 18% 18%, rgba(125, 211, 252, 0.10), transparent 30%)"
        in content
    )
    ...
    assert re.search(
        r"#motivation\s*\{[^}]*var\(--nav-text\)", content
    ), "#motivation should use nav text color"
    # ensure panel backgrounds use var(--surface)
    assert "dark-mode .page-header" in content
    panel_rule = [
        line for line in content.splitlines() if "dark-mode .page-header" in line
    ]
