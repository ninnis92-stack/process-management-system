...
from app.models import User, Request as ReqModel, FormTemplate, FormField, DepartmentFormAssignment, StatusBucket, BucketStatus
...
with app.app_context():
    db.drop_all()
    db.create_all()
    u = User(email='admin@example.com', password_hash=generate_password_hash('secret', method='pbkdf2:sha256'), department='B', is_active=True, is_admin=True)
    db.session.add(u)
    db.session.commit()
    ...
    client.post('/auth/login', data={'email':'admin@example.com','password':'secret'}, follow_redirects=True)
