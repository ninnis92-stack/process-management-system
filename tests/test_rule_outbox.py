...
def test_rule_fires_and_enqueues_event(app):
    ...
    from app.models import AutomationRule, IntegrationEvent
    ...
    with app.app_context():
        ...
        # create a request (ensure non-nullable `due_at` is provided)
        req = ReqModel(
            title="T",
            request_type="X",
            description="d",
        ...
        r = AutomationRule(
            name="t",
            triggers_json=["manual_fire"],
            conditions_json={},
            actions_json=[
                {"action": "webhook", "event_name": "automation.test.webhook"}
            ],
            is_active=True,
        )
        db.session.add(r)
        ...
        ev = IntegrationEvent.query.filter_by(
            event_name="automation.test.webhook"
        ).first()
