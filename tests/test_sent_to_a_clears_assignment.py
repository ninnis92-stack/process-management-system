...
def test_sent_to_a_clears_assignment_and_notifies(app, client):
    ...
    with app.app_context():
        ...
        ac = AuditLog.query.filter_by(
            request_id=r.id, action_type="assignment_changed"
        ).first()
        assert ac is not None

        # previous assignee should have received a notification about cleared assignment
        n = Notification.query.filter_by(
            user_id=assignee.id, request_id=r.id, type="assignment_cleared"
        ).first()
