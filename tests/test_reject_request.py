...
def test_reject_request_requires_reason_and_closes(app, client):
    with app.app_context():
        u = User(email='b_assignee@example.com', password_hash=generate_password_hash('secret'), department='B', is_active=True)
        submitter = User(email='submitter@example.com', password_hash=generate_password_hash('secret'), department='A', is_active=True)
    ...
    rv = client.post(f'/requests/{req_id}/reject', data={'reject_reason': ''}, follow_redirects=True)
    assert rv.status_code == 200
    assert b'reason is required' in rv.data.lower()

    # Reject with reason should close request and create public comment
    rv = client.post(f'/requests/{req_id}/reject', data={'reject_reason': 'Invalid donor mapping'}, follow_redirects=True)
    assert rv.status_code == 200
