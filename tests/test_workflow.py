...
def test_workflow_spec_transitions(app):
    with app.app_context():
        ...
        spec = {"transitions": [{"from": "NEW_FROM_A", "to": "UNDER_REVIEW"}]}
...
def test_workflow_spec_override_and_deny(app):
    with app.app_context():
        ...
        spec_override = {
            "mode": "override",
            "transitions": [{"from": "NEW_FROM_A", "to": "UNDER_REVIEW"}],
        }
        wf_o = Workflow(
            name="DeptB Override", department_code="B", spec=spec_override, active=True
        )
        db.session.add(wf_o)
        db.session.commit()
        ...
        spec_deny = {"transitions": [], "deny": [["NEW_FROM_A", "B_IN_PROGRESS"]]}
        wf_d = Workflow(
            name="DeptB Deny", department_code="B", spec=spec_deny, active=True
        )
        db.session.add(wf_d)
        db.session.commit()
