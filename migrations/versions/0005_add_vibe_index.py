...
def upgrade():
    ...
    if conn.dialect.name == 'sqlite':
        with op.batch_alter_table('user') as batch_op:
            batch_op.add_column(sa.Column('vibe_index', sa.Integer(), nullable=True, server_default=sa.text('0')))
            # Remove server_default after creation if desired by downstream migrations
    else:
        op.add_column('user', sa.Column('vibe_index', sa.Integer(), nullable=True, server_default=sa.text('0')))
