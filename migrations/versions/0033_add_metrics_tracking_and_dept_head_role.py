...
def upgrade():
    ...
    if _has_table(conn, "department_editor") and not _has_column(
        conn, "department_editor", "can_view_metrics"
    ):
        op.add_column(
            "department_editor",
            sa.Column(
    ...
    if not _has_table(conn, "metrics_config"):
        op.create_table(
            ...
            sa.Column(
                "lookback_days", sa.Integer(), nullable=False, server_default="30"
            ),
            sa.Column(
                "user_metrics_limit", sa.Integer(), nullable=False, server_default="15"
            ),
            sa.Column(
    ...
    if not _has_table(conn, "process_metric_event"):
        op.create_table(
            ...
            sa.Column("id", sa.Integer(), primary_key=True),
            sa.Column(
                "request_id", sa.Integer(), sa.ForeignKey("request.id"), nullable=False
            ),
            sa.Column(
                "actor_user_id", sa.Integer(), sa.ForeignKey("user.id"), nullable=True
            ),
        ...
        op.create_index(
            "ix_process_metric_event_request_id", "process_metric_event", ["request_id"]
        )
        op.create_index(
            "ix_process_metric_event_actor_user_id",
            "process_metric_event",
            ["actor_user_id"],
        )
        op.create_index(
            "ix_process_metric_event_owner_department",
            "process_metric_event",
            ["owner_department"],
        )
        op.create_index(
            "ix_process_metric_event_event_type", "process_metric_event", ["event_type"]
        )
        op.create_index(
            "ix_process_metric_event_created_at", "process_metric_event", ["created_at"]
        )
...
def downgrade():
    ...
    if _has_table(conn, "process_metric_event"):
        op.drop_index(
            "ix_process_metric_event_created_at", table_name="process_metric_event"
        )
        op.drop_index(
            "ix_process_metric_event_event_type", table_name="process_metric_event"
        )
        op.drop_index(
            "ix_process_metric_event_owner_department",
            table_name="process_metric_event",
        )
        op.drop_index(
            "ix_process_metric_event_actor_user_id", table_name="process_metric_event"
        )
        op.drop_index(
            "ix_process_metric_event_request_id", table_name="process_metric_event"
        )
    ...
    if _has_table(conn, "department_editor") and _has_column(
        conn, "department_editor", "can_view_metrics"
    ):
