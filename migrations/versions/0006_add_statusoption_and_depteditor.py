...
def upgrade():
    ...
    # Create status_option table if not exists
    if not insp.has_table("status_option"):
        op.create_table(
            "status_option",
    ...
    # Create department_editor table if not exists
    if not insp.has_table("department_editor"):
        op.create_table(
            "department_editor",
            sa.Column("id", sa.Integer(), primary_key=True),
    ...
    # Add sales_list_reference column to request table if not exists
    request_columns = [col['name'] for col in insp.get_columns('request')]
    if "sales_list_reference" not in request_columns:
