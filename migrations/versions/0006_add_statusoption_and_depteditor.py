...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('notify_on_transfer_only', sa.Boolean(), nullable=False, server_default=sa.text('0')),
    ...
    op.create_unique_constraint('uq_user_dept_editor', 'department_editor', ['user_id', 'department'])

    # Add sales_list_reference column to request table
    if conn.dialect.name == 'sqlite':
        with op.batch_alter_table('request') as batch_op:
            batch_op.add_column(sa.Column('sales_list_reference', sa.String(200), nullable=True))
    else:
        op.add_column('request', sa.Column('sales_list_reference', sa.String(200), nullable=True))
