...
def upgrade():
    try:
        ...
        op.add_column('user', sa.Column('totp_enabled', sa.Boolean(), nullable=False, server_default=sa.text('0')))
    except Exception:
        conn = op.get_bind()
        try:
            ...
            pass
        try:
            conn.execute(sa.text("ALTER TABLE user ADD COLUMN totp_enabled INTEGER DEFAULT 0"))
        except Exception:
            pass
